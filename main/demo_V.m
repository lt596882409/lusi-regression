...
% ---- Model Settings ----
model_config = struct(...
    'VR', struct('fun', @VR, 'params', {'C', 'sigma_V', 'sigma_K'}, 'name', 'VR'), ...
    'RUSI_V', struct('fun', @RUSI_M, 'params', {'C', 'sigma_V', 'sigma_K', 'sigma_P', 'tau'}, 'name', 'RUSI_V') ...
    );
...
V_cache = containers.Map('KeyType','double','ValueType','any');
best_score = inf;
[K, C, V] = meshgrid(K_range, C_range, V_range);
param_grid = [K(:), C(:), V(:)];
scores = nan(size(param_grid,1), 1);
for j = 1:size(param_grid, 1)
    current_params = struct('Sigma_K', param_grid(j,1), 'C', param_grid(j,2), 'Sigma_V', param_grid(j,3));
    if ~isKey(V_cache, current_params.Sigma_V)
        V_cache(current_params.Sigma_V) = V_matrix(Train.X, current_params.Sigma_V);
    end
    current_params.V = V_cache(current_params.Sigma_V);
    current_params.Kertype = 'rbf';
    scores(j) = cross_validate(model_config.("VR"), Train, current_params, k);
...
while flag
    ...
    [K, V, C, Tau] = ndgrid(K_range_si, V_range, C_range_si, tau_range);
    param_grid = [K(:), V(:), C(:), Tau(:)];
    scores = nan(size(param_grid,1), 1);
    for j = 1:size(param_grid, 1)
        current_params = struct('Sigma_K', param_grid(j,1), 'Sigma_V', param_grid(j,2),'C', param_grid(j,3),'Tau', param_grid(j,4));
        if ~isKey(V_cache, current_params.Sigma_V)
            V_cache(current_params.Sigma_V) = V_matrix(Train.X, current_params.Sigma_V);
        end
        current_params.V = V_cache(current_params.Sigma_V);
